...
def ingest_data(data):
    """
    Clean ingestion function for API + MQTT reuse
    """
    buffers.prediction_buffer.append(data)

    #print(
    #    f"mqttttttttttt code "
    #    f"[BUFFER] Device={data['esp_id']} | "
    #    f"RSSI={data['rssi']} | "
...
def on_message(client, userdata, msg):
    try:
        ...
        print(
            f"mqttttttttttt code shlne tany wahda"
...
def start_mqtt_client():
    ...
    try:
        ...
        print(f"[MQTT] ⚠️  Failed to connect to {MQTT_BROKER}:{MQTT_PORT}")
        print(f"[MQTT] Error: {e}")
        print(f"[MQTT] App will continue running. Use fake_mqtt_publisher.py for testing.")

...
try:
    while True:
        ...
        try:
            ...
            r3 = requests.post(INGEST_ENDPOINT, json=rx3_payload, timeout=2)

            if r1.status_code == 200 and r2.status_code == 200 and r3.status_code == 200:
                buf_size = r3.json().get("buffer_size", "?")
                print(f"[OK] Sent RX1 + RX2 + RX3 triplet #{counter:3d} | Buffer: {buf_size}")

                # Print milestone message every 50 packets
                if counter > 0 and counter % 50 == 0:
                    print(f"     [*] After {counter} packets, worker should start processing...")
            else:
                print(f"[ERR] HTTP error: RX1={r1.status_code}, RX2={r2.status_code}, RX3={r3.status_code}")
    ...
    print("=" * 70)
    print(f"[*] Stopped. Sent {counter} RX1/RX2/RX3 triplets")
    print("=" * 70)
